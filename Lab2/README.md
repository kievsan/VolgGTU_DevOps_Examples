...
2.2 Даем репозиторию любое имя, также отметим пунк `Add README file`, добавим файл `.gitignore` для Python и выберем лицензию в пункте `Choose license`. После чего подтверждаем создание.
...
```
После выполнения этой команды в директории проекта появится файл `reqirements.txt`.
...
> ```
> Менеджер пакетов считает из файла все зависимости и установит их в активное виртуальное окружение.
...
6.1 В директории проекта создадим файл `test_app`. Напишем в нем тесты для наших функций, коих у нас одна - `roll_the_dice`.
...
В данном коде мы проверяем что функция `roll_the_dice` возвращает значения в промежутке от 1 включительно до 7 не включая. Делаем это 100 раз в цикле.

> **Обратите внимание**
> Важно заметить что название тестирующей функции начинается с префикса `test_`. Пакет тестирования `pytest` имеет набор шаблонов для имен функций, которые необходимо считать тестирующими. Именно поэтому мы назвали функцию следующим образом.
...
Видим, что `pytest` обнаружил наши тесты, запустил их и вывел результат тестирования. Если хотя бы один из тестов был провален, то все тестирование завершилось бы как непройденное.

Теперь передем к линтингу при помощи `flake8`.

> **Полезно знать**
> Помимо тестирования, существует так же процесс называесый линтингом. Линтинг - статический анализ кода. Линтер - программа осуществляющая линтинг. `flake8` как раз и является одним из популярных линтеров для языка python. Статический анализ кода подразумевает под собой поиск синтаксических или стилистических ошибок в коде. Если с синтаксическими ошибками все очевидно - у языков программирования есть свой синтаксис, который необходимо соблюдать чтобы программа компилировалась или интерпретировалась, то под стилистическими ошибками понимается написание кода не следующего общим соглашениям по упрощению его читабельности и чистоты. Главным таким соглашением для языка python является PEP8.
...
И удалим лишнии пробелы в конце файла `test_app.py`.
...
Однако в ходе разработки мы постоянно пишем новый код или редактируем уже имеющийся и при каждой отправке изменений в репозиторий мы должны не забыть запускать тесты и линтер, чтобы удостовериться что наш код рабочий и написан по стандартам. Как раз таки эта задача, как одна из многих входит в список проблем которые значительно упрощет методология непрерывной интеграции и развертывания (CI/CD). Одним из популярных инструментов предоставляющего функции CI/CD является GitHub Actions.

## Этап 7. Подключение Actions в репозиторий

7.1 Вернемся на страницу репозитория и перейдем в раздел `Actions`.

![](./media/actions-tab.png)

В данном разделе приведено множество инcтрументов для различных нужд, начиная от тестирования и заканчивая развертыванием приложения на кластерах облачных провайдеров. Так как мы лишь знакомимся с GitHub Actions, то будет достаточно использования и чего-то простого. Тестирование и линтинг - то что нужно.
...
В нем описаны инструкции которые будут выполняться по некоторым событиям. События указанным в разделе `on`. В нашем случае это `push` или `pull request` в ветку `master`. Как только происходит любое из этих событий запускаются задачи. Задачи описаны в разделе `jobs`. В нашей конфигурации мы имеем всего одину задачу с именем `build`. Задача имеет платформу на которой будет происходить запуск инструкций, она устанавливается свойством `runs-on`. Далее идут шаги в которых выполняются команды, которые мы уже узнаем - установка зависимостей, запуск линтера и тестов.

Нас вполне устроит такая тестовая конфигурация, не будем ничего менять.

![](./media/workflow-configuration.png)

7.4 В правом верхнем углу нажимаем зеленую кнопку `Commit changes` тем самым подтверждая добавление workflow в проект. Во всплывающем окне подтверждаем внесение изменений напрямую в ветку `master`.
...
Для начала добавим все файлы к коммиту и совершим коммит используя следующие команды:
```
git add .
...
Как мы помним наш workflow сконфигурирован как раз на запуск по событию `push` в ветку `master`.
...
Видим что последний пришедший коммит иницииоровал запуск workflow, который уже успешно завершился.

7.7 Изучим подробнее что происходило внутри запущенного workflow. Для этого перейдем в карточку соответствующую нашему коммиту. 
...
В ходе данной лабораторной работы мы познакомились с инструментом GitHub Actions, с помощью которого можно настраивать и автоматизировать различные операции связанные с анализом, сборкой, доставкой и развертыванием приложения.
